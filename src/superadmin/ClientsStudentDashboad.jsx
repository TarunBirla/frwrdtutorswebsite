...
import {
  Users, BookOpen, GraduationCap, Receipt,
  Wallet, Briefcase, ChevronRight, Calendar,
  Clock, ArrowRight, FileText, Star,
} from "lucide-react";
import {
...
const StatCard = ({ title, value }) => {
  const meta = CARD_META[title] || { bg: BRAND_LIGHT, color: BRAND, icon: Briefcase };
  const Icon = meta.icon;
  return (
    <Card className="p-5 hover:shadow-md transition-all duration-200 cursor-pointer group">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider mb-3" style={{ color: GRAY }}>
            {title}
          </p>
          <p className="text-2xl font-bold" style={{ color: meta.color }}>{value}</p>
        </div>
...
const ClientsStudentDashboard = () => {
      ...
      <div className="flex min-h-screen" style={{ background: BG }}>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col items-center justify-center h-[80vh] gap-4">
            ...
            <p className="text-sm" style={{ color: GRAY }}>Loading dashboard…</p>
          </div>
        </div>
      </div>
    );
  ...
  const rawData = (() => {
    try { return typeof client.raw_data === "string" ? JSON.parse(client.raw_data) : client.raw_data || {}; }
    catch { return {}; }
  })();
  ...
  const cards = [
    ...
    { title: "Payments",    value: `AED ${Number(overview.payments       || 0).toLocaleString()}` },
    { title: "Outstanding", value: `AED ${Number(overview.invoiceBalance || 0).toLocaleString()}` },
  ];

  const paymentData = [
    { name: "Paid",        value: dashboard?.paymentOverview?.paid        || 0 },
    { name: "Outstanding", value: dashboard?.paymentOverview?.outstanding || 0 },
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <Card className="p-0 overflow-hidden">
            <div className="flex items-center justify-between p-6"
              style={{ background: `linear-gradient(135deg, ${BRAND} 0%, #5A58B8 100%)` }}>
              <div className="flex items-center gap-5">
                <div className="relative">
                  <img
                    src={photo || `https://ui-avatars.com/api/?name=${client.first_name}+${client.last_name}&background=EEEDFE&color=3C3A86&size=80`}
            ...
            <div className="grid grid-cols-3 divide-x divide-gray-100" style={{ borderTop: "1px solid #F3F4F6" }}>
              {[
                { label: "Total Lessons",   value: overview.lessons    || 0, sub: `${overview.upcomingLessons || 0} upcoming` },
                { label: "Total Invoices",  value: overview.invoices   || 0, sub: "all time" },
                { label: "Total Paid",      value: `AED ${Number(overview.payments || 0).toLocaleString()}`, sub: "lifetime payments" },
              ].map((s, i) => (
                <div key={i} className="p-4 text-center">
                  <p className="text-lg font-bold" style={{ color: BRAND }}>{s.value}</p>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
              ...
              {students.length === 0 ? <EmptyState text="No students found" /> : (
                <div className="space-y-1">
                  {students.map((student) => {
                    const sp = (() => { try { return typeof student.raw_data === "string" ? JSON.parse(student.raw_data) : student.raw_data || {}; } catch { return {}; } })();
                    ...
                    return (
                      <div key={student.id}
                        ...
                        <div className="flex items-center gap-3">
                          <img
                            src={sPhoto || `https://ui-avatars.com/api/?name=${student.first_name}&background=EEEDFE&color=3C3A86`}
            ...
            {/* Upcoming Lessons */}
            <Card className="p-6">
              <SectionHeader title="Upcoming Lessons" actionLabel="View Calendar →" />
              {upcomingLessons.length === 0 ? <EmptyState text="No upcoming lessons" /> : (
                <div className="space-y-3">
                  {upcomingLessons.map((lesson) => {
                    const date  = new Date(lesson.start_time);
                    const finish = new Date(lesson.finish_time);
                    const rcras = (() => { try { return typeof lesson.rcras === "string" ? JSON.parse(lesson.rcras) : lesson.rcras || []; } catch { return []; } })();
                    const cjas  = (() => { try { return typeof lesson.cjas  === "string" ? JSON.parse(lesson.cjas)  : lesson.cjas  || []; } catch { return []; } })();
                    ...
                    return (
                      <div key={lesson.id}
                        ...
                        <div className="flex-shrink-0 w-12 text-center rounded-xl py-2"
                          style={{ background: BRAND_LIGHT }}>
                          <p className="text-xs font-semibold uppercase" style={{ color: BRAND }}>
                            {date.toLocaleString("default", { month: "short" })}
                          </p>
                          <p className="text-xl font-bold" style={{ color: BRAND }}>{date.getDate()}</p>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-800 truncate">{lesson.topic || "Lesson"}</p>
                          <p className="text-xs text-gray-500 mt-0.5">Student: {studentName}</p>
                          <p className="text-xs text-gray-500">Tutor: {tutorName}</p>
                        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: "1px solid #F3F4F6" }}>
                    {["Invoice #", "Description", "Amount", "Status", "Date Sent", "PDF"].map(h => (
                      <th key={h} className="pb-3 text-left font-medium pr-4"
                ...
                <tbody>
                  {recentInvoices.length === 0 ? (
                    <tr><td colSpan={6}><EmptyState text="No invoices found" /></td></tr>
                  ) : recentInvoices.map((inv) => (
                    <tr key={inv.id}
                      className="hover:bg-gray-50 transition-colors"
                      style={{ borderBottom: "1px solid #F9FAFB" }}>
                      <td className="py-3 pr-4">
                        <span className="font-semibold" style={{ color: BRAND }}>{inv.display_id}</span>
                      </td>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            ...
            {/* Payment Overview */}
            <Card className="p-6">
              <SectionHeader title="Payment Overview" actionLabel="View Payments →" />
              {!hasPaymentData ? <EmptyState text="No payment data" /> : (
                <>
                  ...
                  <div className="mt-4 rounded-xl p-4 flex justify-between items-center"
                    style={{ background: BRAND_LIGHT, border: `1px solid ${BRAND_MID}` }}>
                    <span className="font-semibold text-gray-700">Total Outstanding</span>
                    <span className="font-bold text-xl" style={{ color: BRAND }}>
                      AED {Number(dashboard?.paymentOverview?.outstanding || overview.invoiceBalance || 0).toLocaleString()}
                    </span>
                  </div>
                </>
              )}
            ...
            {/* Active Tutors */}
            <Card className="p-6">
              <SectionHeader title="Active Tutors" actionLabel="View Tutors →" />
              {tutors.length === 0 ? <EmptyState text="No tutors found" /> : (
                <div className="space-y-1">
                  {tutors.map((tutor) => {
                    const tp = (() => { try { return typeof tutor.raw_data === "string" ? JSON.parse(tutor.raw_data) : tutor.raw_data || {}; } catch { return {}; } })();
                    ...
                    return (
                      <div key={tutor.id}
                        ...
                        <div className="flex items-center gap-3">
                          <img
                            src={tPhoto || `https://ui-avatars.com/api/?name=${tutor.first_name}&background=ECFDF5&color=059669`}
                            className="w-12 h-12 rounded-xl object-cover"
                            alt=""
                          />
                          <div>
