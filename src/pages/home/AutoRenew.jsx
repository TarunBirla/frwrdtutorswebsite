...
const AutoRenew = () => {
  ...
  // const getTimeFromKey = (key) => key.split("-").slice(4).join("-");
...
const getTimeFromKey = (key) => {
  ...
  const handleAutoRenew = async () => {
    try {
      ...
      //   (a, b) => new Date(a.key) - new Date(b.date),
      // );
      const sortedSlots = [...bookingData.bookdata].sort((a, b) => {
        const da = new Date(getDateFromKey(a.key) + "T00:00:00");
        const db = new Date(getDateFromKey(b.key) + "T00:00:00");
        return da - db;
      });
      ...
      for (let i = 1; i < sortedSlots.length; i++) {
        const prevDateStr = getDateFromKey(sortedSlots[i - 1].key);
        const currDateStr = getDateFromKey(sortedSlots[i].key);
      ...
      const lastDateObj = new Date(getDateFromKey(lastSlot.key) + "T00:00:00");
      const firstDateObj = new Date(
        getDateFromKey(firstSlot.key) + "T00:00:00",
      );
      ...
      let temp = new Date(baseDate); // baseDate;

      for (let i = 0; i < sortedSlots.length; i++) {
        const slot = sortedSlots[i];
        // const uk = toUKDateTime(slot.date);
        const uk = {
          date: getDateFromKey(slot.key),
          time: getTimeFromKey(slot.key),
        };

        // const d = temp.toISOString().split("T")[0];
        const d = formatLocalDate(temp);
        ...
        if (i < gaps.length) {
          ...
          temp.setUTCDate(temp.getUTCDate() + gaps[i]);
        }
      }
...
======================= */
      ...
      for (let i = 0; i < sortedSlots.length; i++) {
        ...
        console.log("📅 CORRECT BASE DATE:", temp);

        console.log("👨‍🏫 Checking Tutor:", tutorDate);

        const checkDate = formatLocalDate(tutorDate);
        ...
        if (!picked) {
          picked = tutors.find(
            (t) => t.subjects?.includes(subject) || t.main_subject === subject,
          );
        }
        ...
        if (i < gaps.length) {
          // tutorDate.setDate(tutorDate.getDate() + gaps[i]);
          tutorDate.setUTCDate(tutorDate.getUTCDate() + gaps[i]);
        }
      }
      ...
      sortedSlots.forEach((old, i) => {
        ...
        if (i < gaps.length) {
          // buildDate.setDate(buildDate.getDate() + gaps[i]);
          buildDate.setUTCDate(buildDate.getUTCDate() + gaps[i]);
        }
      });
      ...
      const lessonName1 = cleanName(packageForm.lessonData?.studentName?.name1);

      const lessonName2 = cleanName(packageForm.lessonData?.studentName?.name2);
      ...
      {showStudentSelect && (
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center">
          <div className="bg-white w-[90%] max-w-sm p-6 rounded-xl">
            <h3 className="text-center mb-3 font-semibold">Select Student</h3>
