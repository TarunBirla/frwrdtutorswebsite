...
if (studentsCount > 1 && studyType === "no" && selectedStudent) {
  ...
  if (!matched) {
    toast.error("No package for selected student");
    return;
  }
...
const rawLevel = activeStudent.program;
      ...
      for (let slot of sortedSlots) {
        ...
        if (!isAvailable(d, uk.time)) {
          toast.error("Slot not available " + d);
          return;
        }
...
// Fallback: if original not found, pick first
if (!picked) {
  console.warn(
    "⚠️ Original tutor not found, picking first available"
  );
  picked = tutors[0];
}

if (!picked) {
  toast.error(`No tutor available on ${checkDate}`);
  return;
}
