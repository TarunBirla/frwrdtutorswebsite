...
const BookClass = () => {
  ...
  const getAvailableSlotsForDate = (date) => {
    ...
    appointments.forEach((appt) => {
      ...
      // Loop while full 1 hour possible
  ...
  //   const teachers = JSON.parse(localStorage.getItem("selectedTeachers")) || {};
  ...
  const handleToggleSlot = async (date, time, apt_id) => {
    ...
    if (hasConflict(date, time)) {
      toast.error("This time slot conflicts with a slot from another subject.");
      return;
    }
          ...
          {noSlotAvailable && (
            <div className="mt-6 bg-black-50 border border-black-200 rounded-lg p-4 text-center">
              <p className="text-sm text-black-600 font-medium mb-3">
                ⚠️ No slots are currently available for this subject.
              </p>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <button
                            onClick={() => {
                              ...
                              let subjectArray =
                                storedTeachers[selectedSubject] || [];
                              ...
                              Object.keys(patternSlots).forEach((slotKey) => {
                                ...
                                const index = subjectArray.findIndex(
                                  (t) => t.key === slotKey,
                                );

                                if (index === -1) {
                                  subjectArray.push(slotTeacherData);
                                } else {
                                  subjectArray[index] = slotTeacherData;
                                }
                              });

                              storedTeachers[selectedSubject] = subjectArray;

                              localStorage.setItem(
                                "selectedTeachers",
                                JSON.stringify(storedTeachers),
                              );
