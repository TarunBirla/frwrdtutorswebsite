...
const RemainingBookClass = () => {
  const id = localStorage.getItem("remainingId");
  const classbooked = localStorage.getItem("bookedClassess");
  const classremain = localStorage.getItem("remainingClassess");
  console.log({ id, classbooked, classremain });
  ...
  //   const subjects = [
  //     ...(data.subject || []),
  //     ...(data.student1?.subject || []),
  //     ...(data.student2?.subject || []),
  //   ];
  //   return Array.from(new Set(subjects));
  // };

  const getUniqueSubjects = (data, studentType) => {
    let subjects = [];

    if (studentType == 1) {
      subjects = [...(data.student1?.subject || [])];
    } else if (studentType == 2) {
      subjects = [...(data.student2?.subject || [])];
    } else {
      subjects = [
        ...(data.subject || []),
        // ...(data.student1?.subject || []),
  ...
  const handleToggleSlot = async (date, time, apt_id) => {
    ...
    console.log("weekslotcount:-", weekSlotCount);
  ...
  const handleService = async () => {
    try {
      ...
      if (
        id !== null &&
        id !== "" &&
        classbooked !== undefined &&
        classremain !== undefined
      ) {
        console.log("if");
        remainingLessons = Math.max(
          Number(classremain) - Number(bookdata.length),
          0
        );
        book = Math.abs(Number(bookdata.length) + Number(classbooked));
      } else {
        console.log("else");
        remainingLessons = Math.max(
        ...
        book = bookdata.length;
      }
      // Remaining lessons = total lessons - booked lessons
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {lessionData.packageNames.subjects.map((subj, index) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-4 rounded-[8px] text-xs font-medium text-white ${
            ...
            <div className="">
              ...
              popupInfo.teacherData2.length > 0 ? (
                popupInfo.teacherData2.map((tutor, index) => {
                  ...
                  console.log("first:-", popupInfo?.teacher?.id);
...
export default RemainingBookClass;
