...
import SuccessPopup from "../admin/utils/SuccessPopup";
import Select from "react-select";
import CustomActionPopup from "../admin/utils/CustomActionPopup";
...
const AdminNewBooking = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      if (userData?.clientid) {
        ...
        const response = await httpadmin.get(url);
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/admin/new-booking") {
  ...
  const handleNext = async () => {
    ...
    if (students && students.length > 0) {
      try {
        ...
        // alert("Students submitted successfully!");
        const response2 = await httpadmin.get(`/clientsdata/${userData.clientid}`);
...
useEffect(() => {
  ...
  if (showStudentPopup) {
    ...
    if (studyType === "yes") {
      ...
      if (formData.name1) {
        actions.push({
          label: `Student 1 - ${formData.name1}`,
          value: 1,
        });
      }
      if (formData.name2) {
        actions.push({
          label: `Student 2 - ${formData.name2}`,
          value: 2,
        });
      }
    }
    ...
    if (studyType === "no") {
      ...
      if (formData.student1?.name) {
        actions.push({
          label: `Student 1 - ${formData.student1.name}`,
          value: 1,
        });
      }
      if (formData.student2?.name) {
        actions.push({
          label: `Student 2 - ${formData.student2.name}`,
          value: 2,
        });
      }
    }
  ...
  formData.name1,
  formData.name2,
  formData.student1?.name,
  formData.student2?.name,
]);
  ...
  if (value === "1") {
    ...
    setDisbaled(false);
          ...
          {/* Submit Button */}
          <span className="">if you could not find your program or subject, please contact support</span>
  ...
  <button
    ...
    className="bg-[#49479D] text-white text-[22px] py-2 px-8 rounded-[6px] font-normal"
  >
    Support
  </button>
      ...
      {showPopup && (
        <CustomActionPopup
          title="Please select your study type:"
          actions={actions}
      ...
      {showStudentPopup && (
        <CustomActionPopup
          title="Please select your Student"
