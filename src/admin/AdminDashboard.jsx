...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            const res = await httpadmin.get(
                `/client/profile/${userData?.clientid}`,
            );
    ...
    const handleFirstLoginDone = async () => {
        try {
            await httpadmin.post("/client/mark-first-login", {
    ...
    const handlePasswordChange = async () => {
        ...
        try {
            setLoading(true);
            const response = await httpadmin.post("/oldnewpassword", {
                clientid: userData?.clientid,
                oldPassword,
                newPassword,
            });
        ...
        const fetchAppointments = async () => {
            try {
                ...
                if (userData?.studentdetails?.length > 0) {
                    const studentIdList = userData.studentdetails.map(
                        (s) => s.id,
                    );
            ...
            try {
                ...
                if (Array.isArray(data.data[0])) {
                    data.data.forEach((pkg, index) => {
                        try {
                            const parsedPackageForm = JSON.parse(
                                pkg.packageForm,
                            );
                            // console.log(`Package Form ${index + 1}:`, parsedPackageForm);
                            const { lessonData, packageFormData } =
                                parsedPackageForm;
                            console.log(
                                "packageName:-",
    ...
    useEffect(() => {
        const fetchSubjects = async () => {
            try {
                const response = await httpadmin.get(
                    `/contractors?branch_id=${branchid}`,
                );
                if (response.data) {
    ...
    const handleLogout = () => {
        ...
        navigate("/admin/login"); // apna login route rakhna
    };
                                ...
                                onClick={async () => {
                                    try {
                                        ...
                                        if (response.data?.data[0]) {
                                            ...
                                            const lessonData =
                                                packageForm.lessonData;
                                            const packageFormData =
                                                packageForm.packageFormData;
                                            const storedTeacher =
                                                packageForm.storedTeacher;
                                            const storedTeacher2 =
                                                packageForm.storedTeacher2;

                                            const groupedByWeek =
                                                packageForm.groupedByWeek;
