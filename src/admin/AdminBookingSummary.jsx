...
const AdminBookingSummary = () => {
    ...
    const handleBack = (event) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          navigate("/admin/book-class"); // redirect to book class page
        } else {
  ...
  const handleAcceptTerms = async () => {
    ...
    try {
      ...
      await httpadmin.post("/terms/accept", {
        client_id: userData.clientid,
        is_accepted: true,
      });

      // 2️⃣ Send email (Client + Admin)
      await httpadmin.post("/clientemailsend", {
  ...
  const fetchStudentDetail = async (studentId) => {
    try {
      const response = await httpadmin.get(`/appointments/${studentId}`);
      ...
      if (data && data?.data?.results[0]?.service?.id) {
        const studentInfo = await httpadmin.get(
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const response = await httpadmin.get(`/clientsdata/${userData.clientid}`);
  ...
  const handleService = async () => {
    try {
      ...
      for (const [index, contractorObj] of updatedContractorList.entries()) {
        ...
        const serviceRes = await httpadmin.post("/services/", serviceData);
        ...
        const res = await httpadmin.post("/appointments/", appointmentPayload);
